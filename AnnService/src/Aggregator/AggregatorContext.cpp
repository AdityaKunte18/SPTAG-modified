...
RemoteMachine::RemoteMachine()
    : m_connectionID(Socket::c_invalidConnectionID), m_status(RemoteMachineStatus::Disconnected)
{
}

AggregatorContext::AggregatorContext(const std::string &p_filePath) : m_initialized(false)
{
...
bool AggregatorContext::IsInitialized() const
{
    return m_initialized;
}

const std::vector<std::shared_ptr<RemoteMachine>> &AggregatorContext::GetRemoteServers() const
{
    return m_remoteServers;
}

const std::shared_ptr<AggregatorSettings> &AggregatorContext::GetSettings() const
{
    return m_settings;
}

const std::shared_ptr<VectorSet> &AggregatorContext::GetCenters() const
{
    return m_centers;
}
